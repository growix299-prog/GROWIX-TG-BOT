...
import { Search, Filter, Receipt, RefreshCw, Trash2, Clock } from 'lucide-react'

const CountdownButton = ({ createdAt, months }: { createdAt: string, months: number }) => {
  const [show, setShow] = useState(false);

  if (show) {
    const createdDate = new Date(createdAt);
    // Add months (approx 30 days per month)
    const expiryDate = new Date(createdDate.getTime() + (months * 30 * 24 * 60 * 60 * 1000));
    const now = new Date();
    ...
    return (
      <div className={`mt-1 text-[9px] px-1.5 py-0.5 inline-block rounded font-bold ${isExpired ? 'bg-red-950/50 text-red-400 border border-red-500/20' : 'bg-emerald-950/50 text-emerald-400 border border-emerald-500/20'}`}>
  ...
  return (
    <button 
      ...
      className="mt-1 text-[9px] px-1.5 py-0.5 inline-flex items-center gap-1 rounded bg-cyber-fbi border border-cyber-border text-cyan-400 hover:text-cyan-300 hover:border-cyan-500/30 transition-colors font-bold"
    >
                  ...
                  filteredOrders.map((order) => (
                    <tr key={order.id} className="hover:bg-cyber-card/30 transition-all font-medium border-b border-cyber-border/20">
                      ...
                      <td className="py-2.5 px-3 font-bold text-white">
                        <div className="max-w-[200px]" title={order.products?.name}>
                          ...
                          {order.subscription_months > 0 && order.status === 'COMPLETED' && (
                            <CountdownButton createdAt={order.created_at} months={order.subscription_months} />
                          )}
                        </div>
                      </td>
