...
if os.path.exists(env_path):
    with open(env_path, "r", encoding="utf-8") as f:
        for line in f:
            # Strip null bytes and whitespace to handle corrupted encoding
...
async def process_digital_delivery(order_id: str, payment_id: str, amount: float, raw_payload: Dict[str, Any]):
    ...
    # 2. Log Raw Payment Record as verified (Strict Idempotency Check)
    payment_logged = create_payment_record(order_id, payment_id, amount, True, raw_payload)
    if not payment_logged:
        logger.warning(f"Payment {payment_id} for order {order_id} already processed (duplicate webhook). Skipping delivery.")
        return
...
async def process_wallet_deposit(notes: Dict[str, Any], payment_id: str, amount: float, raw_payload: Dict[str, Any]):
    ...
    # Log payment record (Acts as an idempotency check - fails if already logged)
    payment_logged = create_payment_record(payment_id, payment_id, amount, True, raw_payload)
    if not payment_logged:
        logger.warning(f"Payment {payment_id} already processed (duplicate webhook). Skipping wallet credit.")
        return
