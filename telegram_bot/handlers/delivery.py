...
from backend.services.supabase_service import get_db, create_ott_request, get_unused_credential, mark_credential_used, update_order_completed, create_review, deduct_wallet_balance, create_order, refund_wallet_balance, get_wallet_balance
...
async def handle_user_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if context.user_data.get('awaiting_optional_email'):
        ...
        try:
            ...
            # Mark as delivered anyway since credentials were already sent via Telegram
            try:
                supabase.table("orders").update({
                    "delivery_status": "DELIVERED"
                }).eq("id", order_data["order_id"]).execute()
                ...
                pass
            await message.reply_text(
                f"⚠️ <b>Email could not be sent</b>, but your credentials are already delivered in the chat above! 🎉\n\n"
                f"<i>If you need them emailed, please contact support.</i>",
                reply_markup=InlineKeyboardMarkup([
                    [InlineKeyboardButton("✍️ Write a Review", callback_data="write_review")],
                    [InlineKeyboardButton("🏠 Main Menu", callback_data="main_menu")],
                    [InlineKeyboardButton("🛍️ Buy More", callback_data="view_products")]
        ...
        try:
            ...
            if response.data:
                ...
                try:
                    ...
                    if success:
                        logger.info(f"Email successfully delivered to {email} for order {order_data['id']}")
                        await message.reply_text(
                            f"✅ <b>Credentials sent to {email} successfully!</b>\n\nCheck your inbox (and spam folder).",
                            reply_markup=InlineKeyboardMarkup([
                                [InlineKeyboardButton("✍️ Write a Review", callback_data="write_review")],
    ...
    # 3.5 Conversational Flow: Handle Wallet Confirmation
    if context.user_data.get('awaiting_wallet_confirmation'):
        conf_data = context.user_data['awaiting_wallet_confirmation']
        text_lower = text.lower().strip()
        
        if text_lower in ['yes', 'y', 'haa', 'ha', 'yep', 'yeah']:
            product = conf_data['product']
            months = conf_data['months']
            qty = conf_data['qty']
            price = conf_data['price']
            
            # Remove state so it doesn't double-trigger
            ...
            if not success:
                await message.reply_text("❌ Wallet deduction failed. Try again.")
                return
            ...
            if not order_data:
                ...
                await message.reply_text("❌ Order creation failed. Your wallet has been refunded.", parse_mode="HTML")
                return
            ...
            if len(credentials) == qty:
                ...
                try:
                    ...
                    await message.reply_text(text=email_ask_msg, reply_markup=InlineKeyboardMarkup(keyboard), parse_mode="HTML")
                    ...
                    await message.reply_text(f"❌ Delivery failed due to a system error. Your wallet has been refunded.\n\nError: <pre>{html.escape(err_trace[-800:])}</pre>", parse_mode="HTML")
            else:
                ...
                await message.reply_text(text=msg, reply_markup=InlineKeyboardMarkup(keyboard), parse_mode="HTML")
            return
            
        elif text_lower in ['no', 'n', 'nhi']:
            # Cancel purchase
            context.user_data.pop('awaiting_wallet_confirmation', None)
            
            from telegram_bot.handlers.menu import send_products_list
            await message.reply_text("❌ <b>Payment Cancelled.</b>", parse_mode="HTML")
            
            # Send them back to the category products list
            product = conf_data['product']
            await send_products_list(update, context, product['category'])
            return
        else:
            await message.reply_text("❌ Please type YES to confirm or NO to cancel.")
            return
    ...
    # Default fallback: Unrecognized text
    fallback_text = (
        "❌ <b>Couldn't find the product!</b>\n\n"
        "You might have typed the wrong spelling. Please check and type again, or use the menu buttons below:"
    )
