...
async def handle_callback_query(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "check_joined":
        ...
        keyboard = [
            ...
            [InlineKeyboardButton("⚠️ Paid but Wallet empty? Contact Support", url="https://t.me/ur_Growixx222")],
        ...
        keyboard = [
            ...
            [InlineKeyboardButton("❌ Decline", callback_data=f"prod_{product_id}")]
        ]
        ...
        keyboard = [
            ...
            [InlineKeyboardButton("⚠️ Paid but no ID/Pass? Contact Support", url="https://t.me/ur_Growixx222")],
            [InlineKeyboardButton("❌ Cancel Order", callback_data=f"prod_{product_id}")]
        ]
        ...
        try:
            # Check if there are any AWAITING_EMAIL_ONLY orders
            response = supabase.table("orders").select("id").eq("telegram_id", user.id).eq("status", "COMPLETED").eq("delivery_status", "AWAITING_EMAIL_ONLY").execute()
            pending_orders = response.data or []
            
            if not pending_orders and not order_id:
                await query.answer("Email was already submitted or no pending requests!", show_alert=True)
                await query.edit_message_text("✅ Email request completed.", parse_mode="HTML")
                return
